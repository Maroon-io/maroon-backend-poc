...
const TradeAction: React.FC<TradeActionProps> = ({
  type,
  onClick,
  activeTab,
}) => {
  ...
  const handlePlaceOrder = async (type: string) => {
    try {
      ...
      console.log({ selectedToken, baseToken, quoteToken });
      ...
      const response = await axios.post(`/api/order/make-order`, data);

      console.log(response)

      const allOrders: ReturnOrderI[] = response.data;
  ...
  return (
    <TEActionWrapper onClick={handleClick}>
      ...
      <div>
        <Button inactive={activeTab === type} onClick={() => handlePlaceOrder(type)} label={type} />
      </div>
    </TEActionWrapper>
  );
};
